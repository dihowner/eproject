            ...
            <div class="content-page">
                <div class="content">
                    ...
                    <div class="container-fluid">
                        ...
                        <!-- end row -->
						<?php
						if(isset($_REQUEST["edtAds"])) { //Edit Ads...
							$id = $_REQUEST["id"];
							$getAd = $action->query("select * from ads where userid='$uid' and id='$id'"); $getAd->execute();
							$getAds = $getAd->fetch(PDO::FETCH_ASSOC);
						?>
							...
							if(isset($_POST["editAds"])) {
								...
								<?php } else if($isUpload == true) {
									...
									<?php } else {
										//Save to db..
										$saveAds = $action->query("update ads set name='$adsName', content='$adsDesc', images='$adsFileName', url='$adsUrl', department='$deptID' where id='$id'");
										...
										if($saveAds->execute()) { ?>
											<script>
												swal.fire({
													icon: "success",
													title: "Ads Updated",
													text: "Your Ads has been updated successfully"
												}).then(function() {
													window.location = "adsHistory";
												});
											</script>
										<?php } else { ?>
										<div class="col-12">
											<div class="alert alert-danger" style="font-size: 20px">
												<b>Error : </b> We could not update your Ads at the moment, kindly try again later
											</div>
										</div>
									<?php }
									}
									...
									//Save to db..
									$saveAds = $action->query("update ads set name='$adsName', content='$adsDesc', url='$adsUrl', department='$deptID' where id='$id'");
							...
							<div class="row">
								<div class="col-12">
									<form method="post" enctype="multipart/form-data">
										...
										<input type="text" class="form-control" name="adsName" placeholder="Enter Ads Name" value="<?php echo $getAds["name"];?>" required /> <br/>
										
										<label><b>Ads Description</b></label>
										<textarea class="form-control input-lg" name="adsDesc" rows="5" placeholder="Enter Ads description" required><?php echo $getAds["content"];?></textarea><br/>
										
										<label><b>Ads URL</b></label>
										<input type="text" class="form-control" name="adsUrl" placeholder="Enter Ads URL e.g facebook , twitter, whatsapp or Website URL" value="<?php echo $getAds["url"];?>" required /> <br/>
										...
										<select type="text" class="form-control" name="deptID">
											...
											while($loadDepts = $loadDept->fetch(PDO::FETCH_ASSOC)) { ?>
												<option value="<?php echo $loadDepts['id'];?>"<?php echo ($getAds["department"] == $loadDepts['id']) ? "selected='selected'":""; ?>">
													<?php echo $loadDepts['dertmentName'];?>
												</option>
											<?php } ?>
										</select>
										<br/>
										
										<label><b>Ads Image (Leave Empty if you are not changing)</b></label>
										...
										<?php if(!empty($getAds["images"])) { ?>
											<h4><b>Ads Image</b></h4>
											<img src="../uploads/ads/<?php echo $getAds["images"];?>" style="width: 300px; height: 150px"/> <br>
										<?php } ?>
										
										<div class="text-center">
											<button type="submit" name='editAds' class="btn btn-primary btn-lg">
												<b><i class="ti-pencil"></i> Edit Ads</b>
											</button>
										</div>
                        ...
                        <div class="row">
                            <div class="col-12">
								<h4><b> Ads History</b></h4> Here you can see all your Ads History<br><br>
							</div>
						</div>
						
						
						<?php 
						if(isset($_REQUEST["delAds"])) {
							$adsID = $_REQUEST["id"];
							$delAds = $action->query("delete from ads where id='$adsID'");
							if($delAds->execute()) { ?>
								<script>
									swal.fire({
										icon: "success",
										title: "Ads Deleted",
										text: "Your Ads has been deleted successfully"
									}).then(function() {
										window.location = "adsHistory";
									});
								</script>
							...
							<div class="col-12">
								<div class="alert alert-danger" style="font-size: 20px">
									<b>Error : </b> We are unable to complete your request, please try again
								</div>
							</div>
						<?php }
						}
								 ...
								 <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <?php
													$i = 0;
													$getHis = $action->query("select * from ads where userid='$uid' order by id desc"); $getHis->execute();
													...
													<td>
														<?php if($dss["status"] == 1) { ?>
															<button disabled class="btn btn-info">
																<b><i class='ti-pencil'></i> Edit</b>
															</button>
														<?php } else {  ?>
															<a href="?edtAds&id=<?php echo $dss["id"];?>" class="btn btn-info">
																<b><i class='ti-pencil'></i> Edit</b>
															</a>
														<?php } ?>
														<a href="?delAds&id=<?php echo $dss["id"];?>" class="btn btn-danger" onclick="delAds<?php echo $dss['id'];?>(event)">
												...
												<script>
													function delAds<?php echo $dss['id'];?>(ev) {
														ev.preventDefault();
					
														var urlToRedirect = ev.currentTarget.getAttribute('href');
														var adsName = "<?php echo $dss['name'];?>";
														
														swal.fire({
															title: "Are you sure?",
															html: "You are about to delete Ads (<b>"+adsName+"</b>) from your Ads History",
															icon: "warning",
															buttons: true,
															dangerMode: true,
